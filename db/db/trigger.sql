...
DROP TRIGGER IF EXISTS AUTO_CLEASE_PROJECT_DATA_BEFORE_DROPPING $$
CREATE TRIGGER AUTO_CLEASE_PROJECT_DATA_BEFORE_DROPPING 
BEFORE DELETE ON PROJECTS
FOR EACH ROW
BEGIN
	DELETE FROM PROJECT_PARTNER WHERE PROJECT_ID = OLD.PROJECT_ID;
    DELETE FROM PROJECT_USER WHERE PROJECT_ID = OLD.PROJECT_ID;
    DELETE FROM VERSIONS WHERE PROJECT_ID = OLD.PROJECT_ID;
    DELETE FROM TASKS WHERE PROJECT_ID = OLD.PROJECT_ID;
END
$$


DROP TRIGGER IF EXISTS AUTO_CREATE_TASK_MODIFY $$

CREATE TRIGGER AUTO_CREATE_TASK_MODIFY
AFTER UPDATE ON `tasks`
FOR EACH ROW
BEGIN
	IF OLD.task_state <>  NEW.task_state THEN
		CALL ADD_TASK_MODIFY(  );
    END IF;
END
